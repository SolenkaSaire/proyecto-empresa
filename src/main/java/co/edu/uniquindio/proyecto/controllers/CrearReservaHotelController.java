...
@Controller
public class CrearReservaHotelController implements Initializable {
    ...
    @FXML
    private TableColumn<HabitacionHotelData, String> choose_cantidad;
...
/*
                    ...
                    public void updateItem(String item, boolean empty) {
                        ...
                        if (empty) {
                            ...
                            btn.setOnAction(event -> {
                                ...
                                ObservableList<HabitacionHotelData> cartData = gr_choose_tableview.getItems();

                                // Check if the selected room is already in the cart
                                if (cartData.contains(h)) {
                                    // If the room is already in the cart, show a warning message
                                    Alert alert = new Alert(Alert.AlertType.WARNING);
                                    alert.setContentText("La habitación seleccionada ya está en el carrito.");
                                    alert.show();
                                } else {
    ...
    private void crearReservaHotel(List<HabitacionHotelData> listaHabitaciones){
        try {
            ReservaHotel reservaHotel = crearReservaHotel();
            List<HabitacionReserva> listaAux = crearHabitacionesReserva(listaHabitaciones, reservaHotel);
            mostrarMensajeReservaCreada(reservaHotel, listaAux);
        } catch (TransactionSystemException e) {
            manejarExcepcion(e);
        }
    }

    private ReservaHotel crearReservaHotel() {
        ...
        ReservaHotel reservaHotel = new ReservaHotel(LocalDate.now(), gr_checkin_date.getValue(),
                gr_checkout_date.getValue(), impuesto, "Activa", precioTotal, clienteSeleccionado,
                empleadoLogueado, hotelSeleccionado, regimenSeleccionado);

        reservaHotelRepo.save(reservaHotel);
        return reservaHotel;
    }
    ...
    private void mostrarMensajeReservaCreada(ReservaHotel reservaHotel, List<HabitacionReserva> listaAux) {
    ...
    private void verificarReservaHotel() {
        ...
        if (!mensaje.equalsIgnoreCase("")) {
            ...
            crearReservaHotel(listaHabitaciones);
